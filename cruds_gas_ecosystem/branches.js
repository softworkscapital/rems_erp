...
crudsObj.getBranchesByCompanyId = (company_id) =>{
    return new Promise((resolve, reject)=>{
        pool.query('SELECT * FROM branches WHERE company_id = ?',[company_id], (err, results)=>{
            if(err){
                return reject(err);
            }
            return resolve(results);
...
crudsObj.getBranchNameByBranchId = (branch_id) =>{
    return new Promise((resolve, reject)=>{
        pool.query('SELECT branch_name FROM branches WHERE branch_id = ?',[branch_id], (err, results)=>{
